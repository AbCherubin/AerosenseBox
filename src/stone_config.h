...
#include "string.h"
#include "stdarg.h"
#include "stm32f1xx_hal.h" //Modify references to HAL library versions based on series types

#define STONE_Delay HAL_Delay
#define USER_UART USART2 // Replace huart1 with the user's own UART handle
...
#define stone_uart_read_byte(USER_UART) HAL_UART_Receive_IT(&USER_UART, &STONE_RX_BUF[STONE_RX_CNT], 1)
#define stone_uart_read_frame(USER_UART) HAL_UARTEx_ReceiveToIdle_IT(&USER_UART, STONE_RX_BUF, RX_LEN)
...
#define stone_Transmit(BUF, ...)                        \
    ...
    HAL_UART_Transmit(&USER_UART, (uint8_t *)BUF, __VA_ARGS__, 10);
#endif
