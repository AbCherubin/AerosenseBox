...
#endif
  if (STONE_RX_CNT == 3 && STONE_RX_BUF[STONE_RX_CNT - 3] != 'S' && STONE_RX_BUF[STONE_RX_CNT - 2] != 'T' && STONE_RX_BUF[STONE_RX_CNT - 1] != '<')
  {
    illegal_date(3);
  }                                                                                                                                                                                                 // Make a judgment when there are only 3 data in the buffer
  else if (STONE_RX_CNT > 11 && STONE_RX_BUF[STONE_RX_CNT - 5] == '>' && STONE_RX_BUF[STONE_RX_CNT - 4] == 'E' && STONE_RX_BUF[STONE_RX_CNT - 3] == 'T' && do_crc(STONE_RX_BUF, STONE_RX_CNT) == 0) // STONE_RX_CNT>11 The legal instruction length is at least 14, and the last two expressions verify the end of the frame and CRC checksum
  {
#if MCU_ARDUINO || MCU_ESP
  ...
  if (stone_uart_get_flag(USER_UART))
  {
    ...
    while (1)
    {
      ...
      else
      {
        ...
        if (receive_idel_time2 <= receive_idel_time)
          return; // overflow
