...
int data_last_len, widget_last_len, text_last_len;
int widget_len;
int base_offset;
recive_group STONER = {
    0,
...
recive_group datainit(int len)
{
  ...
  {
    memset(STONER.data, NULL, data_last_len); // If it is not empty, release it first
    free(STONER.data);
    STONER.data = NULL;
  }
  STONER.data = (char *)malloc(len * sizeof(char) + 1);
  STONER.data[len] = '\0';
  data_last_len = len;
  if (!STONER.data)
  {
...
recive_group widgetinit(int len)
{
  ...
  STONER.widget = (char *)malloc(len * sizeof(char) + 1);
  STONER.widget[len] = '\0';
  widget_last_len = len;
  if (!STONER.widget)
  {
...
recive_group textinit(int len)
{
  ...
  {
    memset(STONER.text, NULL, text_last_len);
    free(STONER.text);
    STONER.text = NULL;
  }
  STONER.text = (char *)malloc(len * sizeof(char) + 1);
  STONER.text[len] = '\0';
  text_last_len = len;
  if (!STONER.text)
  {
...
void _stone_recive_free(char *name)
{
  if (name != NULL)
  {
    ...
    {
      memset(STONER.data, '\0', data_last_len);
      free(STONER.data);
      STONER.data = NULL;
    }
    ...
    {
      memset(STONER.text, '\0', text_last_len);
      free(STONER.text);
      STONER.text = NULL;
    }
  }
...
void case_data_text(char _type)
{
  ...
  widget_len = strlen(STONER.widget);
